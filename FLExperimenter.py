...
ACTUAL_YIELD_RES_FILE_COL_STR="actual_yield"
...
LOCAL_RES_MAP_COL_NAMES=[dataset.SAMPLE_ID_EXTRA_COLUMN_NAME, \
						spatial.EASTING_COL_NAME, \
						spatial.NORTHING_COL_NAME, \
						FOLD_RES_FILE_COL_STR, \
						ACTUAL_YIELD_RES_FILE_COL_STR, \
...
class FLClient:
	...
	#testCoords: list of coordinates associated to each sample in the test set
	...
	def __init__(self, id,train_X,train_y,test_X,test_y,testCoords,localTestSampleIds):
		self.id = id
		...
		myio.logWrite("client "+str(id)+" first sample train X :"+ str(train_X[0]),myio.LOG_LEVEL_DEBUG)
		myio.logWrite("client "+str(id)+" first sample train y :"+ str(train_y[0]),myio.LOG_LEVEL_DEBUG)
		myio.logWrite("client "+str(id)+" first sample test X :"+ str(test_X[0]),myio.LOG_LEVEL_DEBUG)
		myio.logWrite("client "+str(id)+" first sample test y :"+ str(test_y[0]),myio.LOG_LEVEL_DEBUG)
		...
		if len(testCoords) == 0:
			raise Exception("Cannot have a client with an empty test coordinate list")
	...
	def getNumberOfFeatures(self):
		...
		elif len(self.train_X.shape) == 4: #tensor based model (samples x time step x features)?
			return self.train_X.shape[3]
	...
	def summaryStr(self):		
		...
		resStr = resStr +" 1st test coordinate: "+str(self.testCoords[0])+","
		resStr = resStr +" last test coordinate: "+str(self.testCoords[-1])+"}\n"		
		return resStr
...
def runFLExperiments(configInputPath,outputDir):
	...
	for eix in range(numExperiments):
		...
		try:
			...
			hyperparameterSet = configDF[myio.FEDERATED_LEARNING_HYPERPARAMETER_SET_KEY][eix]
			...
			#check taht algorithm has appropriate tile size length definition
			if tileSize > 1 and not mymodel.isModelWithInputTensors(algName):
				raise Exception("Configuration file issue. Algorithm that expects no multi-dimensional input samples in space had tile size length > 1")
				
			aggregatorFunc = configDF[myio.FEDERATED_LEARNING_AGGREGATOR_KEY][eix]
					
			originalTargetVar=inDF.values[:,-1]
							
			#compute min and max target variable to  unscale the normalized
			#emissiosn for results presentation
			maxTargetVar = max(originalTargetVar)
			minTargetVar = min(originalTargetVar)
				
			#used for model evaluation when computing metrics that need to be same scale and units as target variable
			unscaler = dataset.DataUnscaler(minTargetVar,maxTargetVar,normalizeDataFlag)
			...
			#parition original normalized dataset into local dataset for each client
			clientDFList,clientIdList=  partitionClientData(normDF)
			
			
			numClients=len(clientIdList)
			...
			trimmedClientDFsBeforeSample=[]
			if mymodel.isModelWithInputTensors(algName):
			
				for cdf in clientDFList:
					...
					dataset.removeUnfilledTensorTiles(trimmedDF,tileSize,spatialResolution)
					trimmedClientDFsBeforeSample.append(trimmedDF)
					
					
								
			else:				
				trimmedClientDFsBeforeSample=clientDFList#tabular models ignore missing values in tiles, given that they dont' use tiles as input
			...
			for itIx in range(numIterations):
				...
				trimmedClientDFList.clear()
				
				#extract subset of each client's dataset 
				for trimmedDF in trimmedClientDFsBeforeSample:	
										
					
					#apply random sampling to each client (must be IID)?
					if clientIIDFlag and nClientRandomSamples > 0:
						sampledTrimmedDF=dataset.sampleDataFrame(trimmedDF,nClientRandomSamples)
						...
						trimmedClientDFList.append(sampledTrimmedDF)							
						
					else: #non-IID has no sampling

						trimmedClientDFList.append(trimmedDF)
				...
				for clientSet,foldIx in crossValClientSetCreation(clientIdList,clientDFList,trimmedClientDFList,spatialResolution,algName,nFeatures, tileSize,numClients,outerCVType,numberFLCVFolds):
					...
					#for basic ML and dnn, NO dimension in space, so just nFeatures x  1 =
					if tileSize>1:
					...
					resFileRowPrefix = resFileRowPrefix+str(tileSize)+","#number of input tesnors
					...
					resFileRowPrefix = resFileRowPrefix+str(hyperparameterSet)+","#hyper parameter set chosen by user
					resFileRowPrefix = resFileRowPrefix+str(spatialResolution)+","#temporal resolution
					...
					if expType==CENTRALIZED_LEARNING_STR:
						...
						r2,mse,rmse,mape,execTime,netComCost,pred,tmpCentralizedClientSet= centralizeLearningEval(clientSet,hyperparameterSet,algName,inputShape,learnRateOverride,epochsOverride,batchSizeOverride,unscaler)
					...
					####################
						...
						for learningTypeStr in PREFIX_LOCAL_ENSEMBLE_EXP_TYPE_LIST:
							if learningTypeStr==LOCAL_LEARNING_STR:
								#local learning
								r2List,mseList,rmseList,mapeList,execTimeList,netComCostList,predList = localLearningEval(clientSet,hyperparameterSet,algName,inputShape,learnRateOverride,epochsOverride,batchSizeOverride,unscaler)																														
					...
					####################
						...
						#run model evaluation experiments
						r2List,mseList,rmseList,mapeList,execTimeList,netComCostList,predList  = federatedLearningEval(totalNumTrainSamples,clientSet,hyperparameterSet,algName,aggregatorFunc,numRounds,numClientsPerRound,inputShape,learnRateOverride,epochsOverride,batchSizeOverride,unscaler)
...
#parititions dataset into subsets based on the CLIENT_ID_COL_NAME column
def partitionClientData(normDF):

	if not spatial.CLIENT_ID_COL_NAME in normDF:
		raise Exception("expected client id column named '"+CLIENT_ID_COL_NAME+"' when clustering samples of each client. Missing column.")
		
	normDF = normDF.copy(deep=True)
	normDF.reset_index(drop=True, inplace=True)
	#find number of clients
	#nClients = 0
	
	
	clientIdList=spatial.findListOfClientIDs(normDF)
	
	clientDFs =[] #lists of indices for each fold of client cid
	...
	for	cid in clientIdList: #double check this syntax is fine. what i mean is for	cid in clientIxMap.keys():
		...
		clientDF = normDF.loc[np.array(normDF[spatial.CLIENT_ID_COL_NAME]) == cid]
		clientDF.reset_index(drop=True, inplace=True)
		clientDFs.append(clientDF)		
...
def crossValClientSetCreation(clientIdList,clientDFList,trimmedClientDFList,spatialResolution,algName,nFeatures,tileSize,numClients,crossValType,numFolds):
	...
	for cix in range(len(clientDFList)):
		...
		shuffledIndices = experimenter.createRowIndexOrdering(crossValType,trimmedClientDF,numFolds)				
		...
		for trainIxs,testIxs in experimenter.crossValSplit(crossValType,trimmedClientDF,numFolds,shuffledIndices):		
			...
			if mymodel.isModelWithInputTensors(algName):
				...
				#TODO: CONSIDER BUILDING THE TILES ONCE, AND JUST REFERENCING THEM TO AVOID BUILDING THEM EACH TIME
				
				train_X,train_y,trainCoords=dataset.createTensorSets(clientDF,trimmedClientDF,trainIxs,tileSize,spatialResolution) 				
				test_X,test_y,testCoords=dataset.createTensorSets(clientDF,trimmedClientDF,testIxs,tileSize,spatialResolution) 
				
				#make sure dataset has shape [samples,timesteps,features]
				train_X = train_X.reshape((train_X.shape[0], tileSize, tileSize, nFeatures))
				test_X = test_X.reshape((test_X.shape[0], tileSize, tileSize, nFeatures))			
								
				
				clientTestCoords=testCoords
				
			else:
				
					
				#extract the features X from target  variable y									
				train_X,train_y = dataset.extractTrainTestValues(trainDF,4) #4 non feature columns (sample id and client id, X, Y), since coordinates column removed in the tensor extract process					 				 
				test_X,test_y = dataset.extractTrainTestValues(testDF,4) #4 non feature columns (sample id and client id, X, Y), since coordinates column removed in the tensor extract process					 
				
				eastIx = testDF.columns.get_loc(dataset.EASTING_COL_NAME)
				northIx = testDF.columns.get_loc(dataset.NORTHING_COL_NAME)
				
				clientTestCoords = testDF.values[:,[eastIx,northIx]]
				#clientTestCoords=np.array([testDF[dataset.EASTING_COL_NAME],testDF[dataset.NORTHING_COL_NAME]])
			...
			client = FLClient(cid,np.copy(train_X),np.copy(train_y),np.copy(test_X),np.copy(test_y),np.array(clientTestCoords),np.array(localTestSampleIds)) 			
...
#to a file with coordinates, actual values, and predictions, etc.
...
def populateClientResultMaps(localOutMapList,clientSet,clientPreds,foldIx,unscaler):
	...
	for cix in range(clientSet.getNumberOfClients()):
		...
		#time stamps
		mycommon.listAppend(localOutMap[spatial.EASTING_COL_NAME],client.testCoords[:,0])		
		mycommon.listAppend(localOutMap[spatial.NORTHING_COL_NAME],client.testCoords[:,1])		
		...
		#actual 2no
		mycommon.listAppend(localOutMap[ACTUAL_YIELD_RES_FILE_COL_STR],test_y)
...
def _createPredictionResFileHelper(localOutMapList,expDir,eix,itNum,outputClientFilesFlag):
	if outputClientFilesFlag:
		...
		for cix in range(len(localOutMapList)):		
			...
			localPredictionDF = localPredictionDF.sort_values(by=[dataset.SAMPLE_ID_EXTRA_COLUMN_NAME])
	...
	globalOutDF = globalOutDF.sort_values(by=[dataset.SAMPLE_ID_EXTRA_COLUMN_NAME])
...
def centralizeLearningEval(clientSet,hyperparameterSet,algName,inputShape,learnRateOverride,epochsOverride,batchSizeOverride,unscaler):
	...
	for i in range(clientSet.getNumberOfClients()):
		...
		if i ==0:			
			...
			centrazliedTestCoords=np.copy(client.testCoords)
			...
			centrazliedTestCoords=np.concatenate((centrazliedTestCoords,client.testCoords))
	...
	dummyClient = FLClient(0,np.copy(centralizedTrain_X),np.copy(centralizedTrain_y),np.copy(centralizedTest_X),np.copy(centralizedTest_y),centrazliedTestCoords,centrazliedTestSampleIds) 	
	...
	_,r2,mse,rmse,mape,pred = fitAndEvaluateModel(centralizedTrain_X,centralizedTrain_y,centralizedTest_X,centralizedTest_y,hyperparameterSet,algName,inputShape,learnRateOverride,epochsOverride,batchSizeOverride,unscaler)
...
def localLearningEval(clientSet,hyperparameterSet,algName,inputShape,learnRateOverride,epochsOverride,batchSizeOverride,unscaler):
	...
	for client in clientSet:		
		...
		model,r2,mse,rmse,mape,pred = fitAndEvaluateModel(client.train_X,client.train_y,client.test_X,client.test_y,hyperparameterSet,algName,inputShape,learnRateOverride,epochsOverride,batchSizeOverride,unscaler)
...
def federatedLearningEval(totalNumTrainSamples,clientSet,hyperparameterSet,algName,aggregator,numRounds,numClientsPerRound,inputShape,learnRateOverride,epochsOverride,batchSizeOverride,unscaler):			
	numClients = clientSet.getNumberOfClients()
	totalNetCommunication=0
	...
	if aggregator == "FedAvg":
		startTime = time.time()
		globalModelWrapper = createModel(hyperparameterSet,algName,inputShape,learnRateOverride,epochsOverride,batchSizeOverride) #start with same initilizer weights for all clients
			
		#build the same model for each client
		for client in clientSet:
			localModelWrapper=createModel(hyperparameterSet,algName,inputShape,learnRateOverride,epochsOverride,batchSizeOverride)
...
def fitAndEvaluateModel(train_X,train_y,test_X,test_y,hyperparameterSet,algName,inputShape,learnRateOverride,epochsOverride,batchSizeOverride,unscaler):
	
	#evaluate the model by training using all the trainign data
	model = createModel(hyperparameterSet,algName,inputShape,learnRateOverride,epochsOverride,batchSizeOverride)
...
def createModel(hyperparameterSet,algName,inputShape,learnRateOverride=-1,epochsOverride=-1,batchSizeOverride=-1):

	# static hyperparameter are  chosen from results of preliminary experiments (different sets of hyperparameters for each year)
	hyperParameters=mymodel.getFLStaticModelHyperparameters(algName,hyperparameterSet)
	...
	myio.logWrite("hyperparameters for model "+algName+" WITH INPUT SHAPE "+str(inputShape)+" ARE :"+ str(hyperParameters),myio.LOG_LEVEL_DEBUG)
